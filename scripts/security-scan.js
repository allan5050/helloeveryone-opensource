...
function checkSecrets() {
  ...
  const patterns = [
    ...
    { regex: /sk-ant-[A-Za-z0-9_-]{32,}/g, name: 'Anthropic API Key' },
    {
      ...
      name: 'JWT Token (potential service key)',
    },
  ...
  const sourceFiles = getAllFiles('app')
    .concat(getAllFiles('lib'))
    .concat(getAllFiles('components'))
  ...
  const checks = [
    {
      ...
      pattern: /Cannot favorite yourself|profileId\s*===?\s*user\.id/,
    },
    {
      file: 'app/api/privacy/block/route.ts',
      pattern: /Cannot block yourself|blockedUserId\s*===?\s*currentUser\.id/,
    },
    {
      file: 'app/api/chat/send/route.ts',
      pattern: /Cannot send message to yourself|receiver_id\s*===?\s*user\.id/,
    },
  ]
