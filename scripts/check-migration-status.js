...
async function checkMigrationStatus() {
  ...
  try {
    ...
    if (error && error.message.includes('bio_embedding')) {
      ...
      status.summary.push('Need to add pgvector extension and bio_embedding columns');
    } else {
  ...
  for (const table of tables) {
    try {
      ...
      if (error && error.message.includes('row-level security')) {
        ...
        console.log(`   ⚠️  ${table} may need RLS policies (data accessible without auth)`);
  ...
  if (status.needsPgVector) {
    console.log('1. Apply pgvector migration (20250103000001_add_embeddings.sql)');
  ...
  if (status.needsPgVector || status.needsIndexes || status.needsRLSPolicies) {
    ...
    fs.writeFileSync(path.join(__dirname, '..', 'targeted-migrations.sql'), targetedSQL);
    console.log('   ✅ Created targeted-migrations.sql with only needed migrations\n');
  }
}
