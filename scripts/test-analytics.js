...
async function testAnalytics() {
  ...
  try {
    ...
    console.log(`   ✅ Users endpoint working: ${usersData.users?.length || 0} demo users found`)
  ...
  try {
    ...
    console.log(`   ✅ Match scores endpoint working: ${scoresData.scores?.length || 0} pairs calculated`)

    if (scoresData.scores?.length > 0) {
      const topMatch = scoresData.scores[0]
      console.log(`   Top match: ${topMatch.user1_name} ↔ ${topMatch.user2_name} (${(topMatch.score * 100).toFixed(0)}%)`)
    }
  } catch (error) {
  ...
  try {
    ...
    if (recalcData.metadata) {
      ...
      console.log(`      - Average score: ${recalcData.metadata.avg_score?.toFixed(3)}`)
      console.log(`      - Strong matches (>70%): ${recalcData.metadata.strong_matches}`)
      console.log(`      - Medium matches (50-70%): ${recalcData.metadata.medium_matches}`)
      console.log(`      - Weak matches (<50%): ${recalcData.metadata.weak_matches}`)
    }
  } catch (error) {
