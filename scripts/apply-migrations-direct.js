...
async function runMigrations() {
  ...
  try {
    ...
    console.log(
      `   pgvector extension: ${hasVector ? '✅ Installed' : '❌ Not installed'}`
    )
    ...
    console.log(
      `   bio_embedding column: ${hasEmbedding ? '✅ Exists' : '❌ Missing'}`
    )
    ...
    console.log(
      `   RLS on profiles: ${hasRLS ? '✅ Enabled' : '❌ Disabled'}\n`
    )
    ...
    `)
    console.log(
      `   bio_embedding: ${finalColResult.rows[0].count > 0 ? '✅ Exists' : '❌ Missing'}`
    )
