-- Test data insertion script with properly normalized vectors
...
-- TRUNCATE profiles, events, rsvps, messages, favorites, blocks, meeting_slots, match_scores CASCADE;

-- Function to generate normalized random vector for testing
CREATE OR REPLACE FUNCTION generate_test_embedding()
RETURNS VECTOR(1536)
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    FOR i IN 1..1536 LOOP
        arr := array_append(arr, random() * 2 - 1);  -- Random values between -1 and 1
    END LOOP;
...
-- Insert test profiles with properly normalized embeddings
...
BEGIN
    ...
    RAISE NOTICE 'Total RSVPs: %', (SELECT COUNT(*) FROM rsvps);
    RAISE NOTICE 'Total messages: %', (SELECT COUNT(*) FROM messages);
    RAISE NOTICE 'Total favorites: %', (SELECT COUNT(*) FROM favorites);
    RAISE NOTICE 'Total blocks: %', (SELECT COUNT(*) FROM blocks);
    RAISE NOTICE 'Total meeting slots: %', (SELECT COUNT(*) FROM meeting_slots);
    RAISE NOTICE 'Total match scores: %', (SELECT COUNT(*) FROM match_scores);
END $$;

-- Test vector similarity search (should now return positive values)
SELECT 
...
SELECT 
    ...
    'Alice should not see Charlie in matches due to block' as expected_behavior;
...
DROP FUNCTION IF EXISTS generate_test_embedding();
