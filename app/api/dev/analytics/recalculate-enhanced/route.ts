...
function semanticInterestMatch(interests1: string[], interests2: string[]): number {
  if (!interests1?.length || !interests2?.length) return 0

  // Define semantic clusters
  const clusters: Record<string, string[]> = {
    tech: ['data-science', 'machine-learning', 'software-engineering', 'devops', 'cybersecurity', 'blockchain', 'ai', 'nlp'],
    health: ['healthcare', 'medical-research', 'fitness', 'yoga', 'running', 'pilates', 'nutrition', 'wellness'],
    creative: ['art', 'design', 'photography', 'music', 'writing', 'illustration', 'sketching'],
    social: ['volunteering', 'community', 'mentoring', 'networking', 'social-impact'],
    lifestyle: ['cooking', 'food', 'wine', 'coffee', 'travel', 'gardening', 'baking'],
    entertainment: ['gaming', 'board-games', 'movies', 'reading', 'podcasts', 'sci-fi', 'star-trek', 'star-wars'],
    pets: ['dogs', 'cats', 'pets', 'animals'],
    sports: ['basketball', 'golf', 'cycling', 'hiking', 'climbing', 'triathlon'],
...
function calculateAgeCompatibility(age1: number, age2: number, tolerance: number = 5): number {
...
function calculateEnhancedMatchScore(
  ...
  const { enableDiversity = true, diversityFactor = 0.1, interestWeights } = options

  // 1. Interest similarity (weighted if provided)
  const exactInterestScore = interestWeights
    ? weightedJaccardSimilarity(user1.interests || [], user2.interests || [], interestWeights)
    : calculateJaccardSimilarity(user1.interests || [], user2.interests || [])

  const semanticInterestScore = semanticInterestMatch(user1.interests || [], user2.interests || [])
  ...
  return {
    ...
    insights: generateMatchInsights(user1, user2, finalScore, interestScore, ageScore)
  }
}
...
function generateMatchInsights(
  ...
  if (commonInterests.length > 0) {
    insights.push(`Share ${commonInterests.length} common interests: ${commonInterests.slice(0, 3).join(', ')}`)
  }
  ...
  try {
    ...
    for (let i = 0; i < users.length; i++) {
      for (let j = i + 1; j < users.length; j++) {
        const matchData = calculateEnhancedMatchScore(users[i], users[j], options)
        scores.push({
