...
export default function DimensionGraphs({ users, matchScores, minScore }: DimensionGraphsProps) {
  ...
  useEffect(() => {
    if (!ageRef.current || !locationRef.current || !connectivityRef.current) return
    ...
    const drawAgeDistribution = () => {
      ...
      // Group users by age
      const ageGroups = d3.rollup(users, v => v.length, d => d.age)
    ...
    const drawLocationClustering = () => {
      ...
      locationData.forEach(({ location }) => {
        ...
        const connections = filteredMatches.filter(m =>
          userIds.has(m.user1_id) && userIds.has(m.user2_id)
        ).length
    ...
    const drawConnectivityDistribution = () => {
      ...
      users.forEach(user => {
        const connections = filteredMatches.filter(m =>
          m.user1_id === user.user_id || m.user2_id === user.user_id
        ).length
      ...
      const connectivityData = Array.from(connectivityGroups, ([connections, count]) => ({
        connections,
        count
      ...
      // Draw line
      const line = d3.line<{ connections: number; count: number }>()
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      <div className="bg-white p-4 rounded-lg border">
        ...
        <p className="text-xs text-gray-600 mt-2">
          Shows how users are distributed by age. Taller bars indicate age groups with more users.
        </p>
      </div>
      <div className="bg-white p-4 rounded-lg border">
        <svg ref={locationRef}></svg>
        <p className="text-xs text-gray-600 mt-2">
          Top 5 locations by user count. Numbers show connections within each location.
        </p>
      </div>
      <div className="bg-white p-4 rounded-lg border">
        <svg ref={connectivityRef}></svg>
        <p className="text-xs text-gray-600 mt-2">
          Distribution of connection counts. Shows how many users have X connections.
        </p>
      </div>
    </div>
  )
