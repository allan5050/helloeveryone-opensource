...
const ZIP_COORDINATES: Record<string, { lat: number; lng: number }> = {
  ...
  '94109': { lat: 37.793, lng: -122.4215 }, // Russian Hill
  ...
  // Oakland
  '94601': { lat: 37.78, lng: -122.2166 }, // Fruitvale
  '94606': { lat: 37.7914, lng: -122.2409 }, // Highland
  '94607': { lat: 37.8072, lng: -122.2981 }, // West Oakland
  '94608': { lat: 37.8335, lng: -122.263 }, // Emeryville border
  '94609': { lat: 37.833, lng: -122.253 }, // Temescal
  ...
  '94703': { lat: 37.871, lng: -122.2761 }, // South Berkeley
  '94704': { lat: 37.867, lng: -122.2577 }, // Central Berkeley
  '94705': { lat: 37.8591, lng: -122.242 }, // UC Berkeley South
  '94706': { lat: 37.8805, lng: -122.3041 }, // Northwest Berkeley
  '94707': { lat: 37.893, lng: -122.241 }, // Berkeley Hills
  '94708': { lat: 37.89, lng: -122.264 }, // North Berkeley
  '94709': { lat: 37.8804, lng: -122.2676 }, // Downtown Berkeley
  '94710': { lat: 37.867, lng: -122.302 }, // West Berkeley

  // San Jose
  '95110': { lat: 37.3382, lng: -121.899 }, // Downtown San Jose
  '95112': { lat: 37.3564, lng: -121.8881 }, // North San Jose
  '95113': { lat: 37.3334, lng: -121.885 }, // Downtown San Jose
  '95116': { lat: 37.354, lng: -121.8463 }, // East San Jose
  '95117': { lat: 37.3089, lng: -121.9525 }, // Campbell border
  '95118': { lat: 37.2486, lng: -121.8944 }, // Almaden Valley
  '95119': { lat: 37.229, lng: -121.7846 }, // South San Jose
  '95120': { lat: 37.218, lng: -121.8616 }, // Almaden

  // Palo Alto
  '94301': { lat: 37.4435, lng: -122.1495 }, // Palo Alto
  '94303': { lat: 37.417, lng: -122.126 }, // East Palo Alto
  '94304': { lat: 37.408, lng: -122.117 }, // East Palo Alto
  '94305': { lat: 37.425, lng: -122.175 }, // Stanford
  '94306': { lat: 37.377, lng: -122.111 }, // South Palo Alto
...
export function getNearbyZips(
  ...
  for (const candidateZip of Object.keys(ZIP_COORDINATES)) {
    if (
      candidateZip !== zip &&
      areZipsNearby(zip, candidateZip, maxDistanceKm)
    ) {
 ...
 */
export function clusterZipCodes(
  zips: string[],
  ...
  for (const zip of zips) {
    ...
    for (const otherZip of zips) {
      if (
        !assigned.has(otherZip) &&
        areZipsNearby(zip, otherZip, maxDistanceKm)
      ) {
