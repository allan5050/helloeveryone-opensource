...
class QueryTool {
  ...
  async runCommonQuery(queryName, ...args) {
    const queries = {
      ...
      // User queries
      users:
        'SELECT id, display_name, role, created_at FROM profiles ORDER BY created_at DESC LIMIT $1',
      ...
      upcoming_events:
        'SELECT * FROM events WHERE start_time > NOW() ORDER BY start_time LIMIT $1',
      past_events:
        'SELECT * FROM events WHERE end_time < NOW() ORDER BY end_time DESC LIMIT $1',
      ...
      user_matches:
        'SELECT * FROM match_scores WHERE profile1_id = $1 OR profile2_id = $1 ORDER BY score DESC',

      // Message queries
      recent_messages:
        'SELECT * FROM messages ORDER BY created_at DESC LIMIT $1',
      conversation:
        'SELECT * FROM messages WHERE (sender_id = $1 AND recipient_id = $2) OR (sender_id = $2 AND recipient_id = $1) ORDER BY created_at',
    ...
    if (!template) {
      throw new Error(
        `Unknown query: ${queryName}. Available: ${Object.keys(queries).join(', ')}`
      )
    }
